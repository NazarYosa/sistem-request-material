...
function App() {
  ...
  const processSheet = (rows, sheetName, markers) => {
    let headerRow = -1, colNo = -1, colPartName = -1, colPartNo = -1, dateColIndex = -1, offsetSak = -1, offsetKg = -1;
    ...
    for (let i = 0; i < Math.min(50, rows.length); i++) {
      ...
      row.forEach((cell, idx) => {
        ...
        if (val === "NO" || val === "NO.") colNo = idx;
        if (val.includes("PART NAME")) colPartName = idx;
        if (val.includes("PART NO") || val.includes("PART NUMBER")) colPartNo = idx;
      });
      row.forEach((cell, idx) => {
        if (!cell || dateColIndex !== -1) return;
        const valStr = String(cell).trim();
        const isDateMatch = valStr === markers.dayStr || valStr === `0${markers.dayStr}` || valStr.includes(markers.fullDate);

        if (isDateMatch) {
          const nextDayStr = String(parseInt(markers.dayStr) + 1);
          if (rows[i + 1] && String(rows[i + 1][idx]).trim() === nextDayStr) return;

          let isHorizontalSequence = false;
          if (row[idx + 1]) {
            const valRight = String(row[idx + 1]).trim();
            if (valRight === nextDayStr || valRight.includes(nextDayStr)) isHorizontalSequence = true;
          }

          let foundSak = -1, foundPlan = -1, foundKg = -1;
          for (let r = 0; r <= 8; r++) {
            if (!rows[i + r]) continue;
            for (let off = -1; off <= 5; off++) {
              const checkIdx = idx + off;
              if (checkIdx < 0 || !rows[i + r][checkIdx]) continue;
              const subVal = String(rows[i + r][checkIdx]).toUpperCase().trim();
              if (["SAK", "BOX", "PACK"].some((k) => subVal.includes(k))) { if (foundSak === -1) foundSak = off; } 
              else if (["PLAN", "QTY", "PCS", "PROD", "TARGET"].some((k) => subVal.includes(k))) { if (foundPlan === -1) foundPlan = off; } 
              else if (["KG", "BERAT", "MAT", "WEIGHT"].some((k) => subVal.includes(k))) { foundKg = off; }
            }
          }

          if (foundSak !== -1 || foundPlan !== -1 || foundKg !== -1 || isHorizontalSequence || valStr.length > 5 || valStr.includes("-")) {
            headerRow = i; dateColIndex = idx;
            if (foundSak !== -1) offsetSak = foundSak; else if (foundPlan !== -1) offsetSak = foundPlan; else offsetSak = 0;
            offsetKg = foundKg !== -1 ? foundKg : -1;
          }
        }
      });
    ...
    if (dateColIndex === -1 || colPartName === -1) return [];
    if (offsetSak === -1 && offsetKg === -1) { offsetKg = 2; offsetSak = 3; }

    const results = [];
    let currentPartName = null, currentPartNo = "-", currentNo = 9999;
    ...
    for (let i = headerRow + 1; i < rows.length; i++) {
      ...
      const cellNo = String(row[colNo] || "").trim();
      const isNewNumber = cellNo.length > 0 && !isNaN(parseFloat(cellNo));
      let cellName = String(row[colPartName] || "").replace(/\s+/g, " ").trim();

      const isHeader = ["PART NAME", "TOTAL", "SUB TOTAL", "MODEL", "ACTUAL"].some((x) => cellName.toUpperCase().includes(x));
      const isModelCode = /^\(.*\)$/.test(cellName);
      const looksLikePartNo = /[0-9]/.test(cellName) && /-/.test(cellName) && !cellName.includes(" ");
      const isShortCodeWithNum = cellName.length <= 5 && /[0-9]/.test(cellName);
      const isTooShort = cellName.length < 2;
      const isZero = cellName === "0";
      const isValidName = cellName && !isHeader && !isModelCode && !looksLikePartNo && !isShortCodeWithNum && !isTooShort && !isZero;

      if (isNewNumber) {
        if (cellName && !isHeader && !isModelCode) {
          currentPartName = cellName; currentNo = parseFloat(cellNo);
          const rawNo = String(row[colPartNo] || "").trim();
          currentPartNo = rawNo.length > 3 ? rawNo : "-";
        }
      } else {
      ...
      let isActualRow = false;
      for (let c = 0; c < 10; c++) {
        if (String(row[c]).toUpperCase().trim() === "ACTUAL" || String(row[c]).toUpperCase().trim() === "ACT") { isActualRow = true; break; }
      }
      if (isActualRow) continue;

      let finalSak = 0, displayKg = 0;
      if (offsetKg !== -1) {
        let val = parseFloat(String(row[dateColIndex + offsetKg]).replace(",", "."));
        if (!isNaN(val) && val > 0) displayKg = val;
      }
      if (offsetSak !== -1) {
        let val = parseFloat(String(row[dateColIndex + offsetSak]).replace(",", "."));
        if (!isNaN(val) && val > 0) finalSak = val;
      }

      if (finalSak > 0) {
        results.push({ machine: sheetName.replace(".csv", "").replace(".xlsx", "").trim(), no: currentNo, partName: currentPartName, partNo: currentPartNo, rawSak: finalSak, rawKg: displayKg });
      }
    }
    return results;
  };
  ...
  const aggregateData = (rawData) => {
    ...
    rawData.forEach((item) => {
      const key = `${item.machine}__${item.partName}`;
      if (!grouped[key]) {
        grouped[key] = { machine: item.machine, no: item.no, partName: item.partName, partNo: item.partNo, totalRawSak: 0, totalRawKg: 0 };
      }
      grouped[key].totalRawSak += item.rawSak;
      grouped[key].totalRawKg += item.rawKg;
    });
    ...
    return Object.values(grouped).map((item) => {
      ...
      return {
        id: Math.random().toString(36),
        machine: item.machine, no: item.no, partName: item.partName, partNo: extraData.partNo || item.partNo,
        inputSak: item.totalRawSak, inputKg: item.totalRawKg, inputPlan: calculatedPlan, totalQty: totalQty,
        jmlLabel: jmlLabel, recycleInput: 0, isExcluded: false,
      };
    });
  };
        ...
        try {
          ...
          workbook.SheetNames.forEach((sheetName) => {
            if (sheetName.trim().toUpperCase().startsWith("M")) {
              ...
              const result = processSheet(jsonData, sheetName, markers);
              if (result.length > 0) extractedData = [...extractedData, ...result];
            }
          });
  ...
  const handleFilePick = async () => {
    try {
      if (!window.showOpenFilePicker) {
        alert("Browser Anda tidak mendukung fitur Live Auto-Sync! Gunakan Chrome atau Edge versi Desktop.");
        return;
      }
  ...
  const handleEditDb = (key) => {
    ...
    setInputForm(data);
    setEditingKey(key);
    if (inputFormRef.current) inputFormRef.current.scrollIntoView({ behavior: "smooth", block: "center" });
  };
  ...
  const toggleExcludePart = (id) => {
    setDataMaterial((prev) => prev.map((item) => item.id === id ? { ...item, isExcluded: !item.isExcluded } : item ));
  };
  ...
  const handlePrintRequest = (item) => {
    ...
    for (let i = 0; i < totalBox; i++) {
      ...
      labels.push({
        ...item, partNameExcel: item.partName, partNoMain: extraData.partNo || item.partNo, materialName: extraData.materialName || "-", partNoMaterial: extraData.partNoMaterial || "-", materialName2: extraData.materialName2 || "", partNoMaterial2: extraData.partNoMaterial2 || "", color: extraData.color || "BLACK", model: extraData.model || "-", qtyDisplay: qtyDisplay, totalDisplay: totalDisplay, boxKe: i + 1, totalBox: totalBox,
      });
      remainingPlan -= currentBoxTotal;
    }
  ...
  const handlePrintLabel = (item, type) => {
    ...
    switch (type) {
      case "GEN":
        targetName = extraData.partNameHgs; targetHgs = extraData.partNoHgs; targetFg = extraData.finishGood; targetQr = extraData.qrHgs; targetImg = extraData.imgHgs; break;
      case "ASSY_GEN":
        targetName = extraData.partAssyName; targetHgs = extraData.partAssyHgs; targetFg = extraData.partAssyFg; targetQr = extraData.qrAssy; targetImg = extraData.imgAssy; break;
      case "ASSY_L":
        targetName = extraData.partAssyNameLeft; targetHgs = extraData.partAssyHgsLeft; targetFg = extraData.partAssyFgLeft; targetQr = extraData.qrAssyL; targetImg = extraData.imgAssyL; break;
      case "ASSY_R":
        targetName = extraData.partAssyNameRight; targetHgs = extraData.partAssyHgsRight; targetFg = extraData.partAssyFgRight; targetQr = extraData.qrAssyR; targetImg = extraData.imgAssyR; break;
      case "TAG_L":
        targetName = extraData.partNameHgsLeft; targetHgs = extraData.partNoHgsLeft; targetFg = extraData.finishGoodLeft; targetQr = extraData.qrTagL; targetImg = extraData.imgTagL; break;
      case "TAG_R":
        targetName = extraData.partNameHgsRight; targetHgs = extraData.partNoHgsRight; targetFg = extraData.finishGoodRight; targetQr = extraData.qrTagR; targetImg = extraData.imgTagR; break;
      default: return;
    }

    if (!targetName && !targetHgs) {
      if (!window.confirm(`Data teks untuk tipe ${type} kosong. Tetap print?`)) return;
    }
    ...
    for (let i = 1; i <= totalBox; i++) {
      labels.push({
        machine: item.machine, partName: targetName || "-", hgs: targetHgs || "-", fg: targetFg || "-", qr: targetQr, img: targetImg, partNo: extraData.partNo, model: extraData.model, qty: stdPack, boxKe: i, totalBox: totalBox,
      });
    }
