...
//             // Logic membersihkan nama mesin
//             const machineNameClean = machine.replace(/^M/i, "");

//             return (
//               <div key={machine} className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden flex flex-col">
                
//                 {/* HEADER MESIN */}
//                 <div className="bg-gray-100 px-5 py-4 border-b border-gray-300 flex justify-between items-center">
//                   <div className="flex items-center gap-3">
//                     <span className="font-black text-slate-800 text-3xl tracking-tighter">{machineNameClean}</span>
//                   </div>
...
//                         {totalPlanKg.toLocaleString("id-ID", { maximumFractionDigits: 0 })}
//                       </span>
//                     </div>
//                   </div>
//                 </div>
...
//                       <tr>
//                         <th className="px-4 py-3 w-[40%] uppercase text-xs">Part Name / Material</th>
...
//                         <th className="px-2 py-3 text-center border-l border-slate-200 w-[10%] uppercase text-xs bg-emerald-50 text-emerald-700">Rec</th>
//                         <th className="px-2 py-3 text-center border-l border-slate-200 w-[10%] uppercase text-xs">Total</th>
//                         <th className="px-4 py-3 text-right w-[18%] uppercase text-xs">Action</th>
//                       </tr>
//                     </thead>
...
//                                     <span className="font-bold text-slate-800 text-base break-words leading-tight group-hover:text-blue-700 transition-colors">
//                                       {item.partName}
//                                     </span>
//                                     {dbData && (
//                                       <span
//                                         className="bg-emerald-500 text-white text-[9px] font-black px-1.5 py-0.5 rounded shadow-sm"
//                                         title="Data tersedia di Database"
//                                       >
//                                         DB
//                                       </span>
//                                     )}
//                                   </div>
//                                   {/* List Material (Dengan Background, Lebar Dinamis, Font Lebih Gede) */}
//                                   <div className="flex flex-col gap-1 w-full mt-0.5 items-start">
//                                     {dbData ? (
//                                       <>
//                                         {/* Material 1 */}
//                                         {dbData.materialName && (
//                                           <div className="w-fit bg-slate-100 border border-slate-200 rounded px-2 py-1 text-xs font-medium text-slate-700 whitespace-normal leading-tight">
...
//                                       </>
//                                     ) : (
//                                       // Peringatan jika belum input DB
//                                       <span className="text-xs text-red-500 font-bold italic mt-1 bg-red-50 px-2 py-0.5 rounded w-fit border border-red-100">
//                                         ⚠️ Belum Input DB
//                                       </span>
//                                     )}
//                                   </div>
...
//                                   type="number"
//                                   min="0"
//                                   className="w-12 text-center text-sm font-bold text-emerald-700 border-2 border-emerald-100 rounded-md py-1 focus:ring-2 focus:ring-emerald-500 outline-none bg-white shadow-sm"
//                                   value={
//                                     item.recycleInput === 0
//                                       ? ""
//                                       : item.recycleInput
...
//                                     className="bg-white border-2 border-slate-200 hover:border-blue-500 hover:text-blue-600 text-slate-600 text-xs font-bold px-3 py-1.5 rounded-md shadow-sm w-full max-w-24 transition-all active:scale-95"
//                                   >
//                                     📄 REQ
//                                   </button>
//                                   <button
//                                     onClick={() => setActiveDropdown(item.id)}
//                                     className="bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-md w-full max-w-24 transition-all active:scale-95"
//                                   >
//                                     🏷️ LBL
//                                   </button>
//                                 </div>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {machineItems
                        ...
                        .map((item, idx) => {
                          ...
                          return (
                            <tr key={idx} className="hover:bg-blue-50 transition-colors group relative">
                              {/* 1. Part Details dengan Indicator Bar */}
                              <td className="p-0 align-middle relative">
                                {/* INDICATOR BAR (Hijau jika ada di DB) */}
                                {isRegistered && (
                                  <div className="absolute left-0 top-0 bottom-0 w-1.5 bg-emerald-500 shadow-[2px_0_4px_rgba(16,185,129,0.2)]"></div>
                                )}
                                
                                <div className={`flex flex-col gap-1.5 py-3 ${isRegistered ? 'pl-5' : 'pl-4'} pr-4`}>
                                  <span className="font-bold text-slate-800 text-base wrap-break-word leading-tight group-hover:text-blue-700 transition-colors">
                                    {item.partName}
                                  </span>
                              ...
                              <td className="px-2 py-3 text-center border-l border-slate-100 align-middle">
                                {item.inputPlan > 0 ? (
                                  <span className="font-bold text-slate-700 bg-slate-100 px-2 py-1 rounded text-sm">{item.inputPlan}</span>
                                ) : <span className="text-slate-300">-</span>}
                              </td>

                              <td className="px-2 py-3 text-center border-l border-slate-100 align-middle">
                                <span className="text-xs font-bold text-slate-500">{item.inputKg > 0 ? item.inputKg.toLocaleString("id-ID", { maximumFractionDigits: 1 }) : "-"}</span>
                              </td>
                              ...
                              <td className="px-2 py-3 text-center border-l border-slate-100 align-middle bg-emerald-50/20">
                                <input
                                  type="number"
                                  min="0"
                                  className="w-12 text-center text-sm font-bold text-emerald-700 border-2 border-emerald-100 rounded-md py-1 focus:ring-2 focus:ring-emerald-500 outline-none bg-white"
                              ...
                              <td className="px-2 py-3 text-center border-l border-slate-100 align-middle">
                                <span className="font-black text-slate-900 text-lg">{item.totalQty}</span>
                              </td>

                              <td className="px-4 py-3 text-right align-middle">
                                <div className="flex flex-col gap-2 items-end">
                                  <button onClick={() => handlePrintRequest(item)} className="bg-white border-2 border-slate-200 hover:border-blue-500 hover:text-blue-600 text-slate-600 text-xs font-bold px-3 py-1.5 rounded-md shadow-sm w-full max-w-24 active:scale-95 transition-all">📄 REQ</button>
                                  <button onClick={() => setActiveDropdown(item.id)} className="bg-slate-800 hover:bg-slate-900 text-white text-xs font-bold px-3 py-1.5 rounded-md shadow-md w-full max-w-24 active:scale-95 transition-all">🏷️ LBL</button>
                                </div>
                              </td>
                            </tr>
                          );
